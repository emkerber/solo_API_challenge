$(document).ready(function () { //initiates jQuery
  ...
  $('button').on('click', function(event) {
    ...
    $('#searchResult .picture').empty(); //to clear previous searches
    $('#searchResult .text').empty(); //to clear previous searches
    var input = $('#searchInput').val(); //assigns the text input to a value
    console.log('http:/' + '/www.' + 'omdbapi.com/?s=' + input);
    $.get('http:/' + '/www.' + 'omdbapi.com/?s=' + input) //searches the API for the movie
    .then(function(data) { //appends movie information to the DOM
      $('#searchResult .picture').append('<img src=\"' + data.Search[0].Poster + '\" alt=\"' + data.Search[0].Title + '\">');
      $('#searchResult .text').append('<p>' + data.Search[0].Title + '</p><p> Year released: ' + data.Search[0].Year + '</p><p>Rated ' + data.Search[0].Rated + '</p><p>Plot: ' + data.Search[0].Plot + '</p>');
    })
